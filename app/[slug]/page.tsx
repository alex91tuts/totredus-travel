...
export async function generateStaticParams() {
    ...
    for (const country of countries) {
        params.push({ slug: country.toLowerCase().replace(/\s+/g, '-') })
    }
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (isCountry || isCity) {
        ...
        if (posts.length > 0) {
            ...
            if (isCity) {
                const postCity = firstPost.frontmatter.oras || firstPost.frontmatter.city
    ...
    if (!post) {
        return {
            ...
            description: 'Articolul solicitat nu a putut fi găsit.',
        }
    }
                                ...
                                <Breadcrumb
                                    ...
                                    items={[
                                        ...
                                        ...(normalizeLocation(post.frontmatter.tara || post.frontmatter.country) ? [{
                                            ...
                                            href: `/${normalizeLocation(post.frontmatter.tara || post.frontmatter.country).toLowerCase().replace(/\s+/g, '-')}`
                                        }] : []),
                                        ...(normalizeLocation(post.frontmatter.oras || post.frontmatter.city) ? [{
                                            ...
                                            href: `/${normalizeLocation(post.frontmatter.oras || post.frontmatter.city).toLowerCase().replace(/\s+/g, '-')}`
                                        }] : []),
                                        ...
                                        <div className="space-y-4">
                                            {relatedPosts.map((relatedPost) => (
                                                <Link
                                                    ...
                                                    href={`/${relatedPost.frontmatter.slug}`}

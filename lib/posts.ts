...
function normalizeFrontmatter(
  ...
  if (data.folder) {
    ...
    const imageFileName = data['imagine-big'] || data.imagine || data['imagine-mobile'] || data.image || data.meta_image
    if (imageFileName) {
  ...
  const normalized: PostFrontmatter = {
    ...
    date: data.date || data['date-modified'] || new Date().toISOString().split('T')[0],
        ...
        return `
          <div class="my-6 sm:my-8 relative not-prose">
            <div class="flex items-center gap-3 p-4 sm:p-5 bg-card border-l-4 border-primary rounded-r-lg shadow-sm hover:shadow-md transition-all duration-300 group">
              <div class="flex-shrink-0 w-10 h-10 bg-muted rounded-full flex items-center justify-center">
                <svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function getRelatedPostsByCity(city: string, currentSlug: string, locale: Locale, limit: number = 8): Post[] {
  if (!city) return []
